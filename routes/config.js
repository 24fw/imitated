router.all(/^\/admin\/config$/, async(req, res, next) => {
	...
	var content = `
		<form method=post class=settings-section>
			...
			<div class=form-group>
				<label class=control-label>대문 문서이름</label>
			...
			<div class=form-group>
				<label class=control-label>편집 주의</label>
				<input class=form-control type=text name=wiki.editagree_text value="${html.escape(config.getString('wiki.editagree_text', `문서 편집을 <strong>저장</strong>하면 당신은 기여한 내용을 <strong>저작권</strong>으로 배포하고 기여한 문서에 대한 하이퍼링크나 URL을 이용하여 저작자 표시를 하는 것으로 충분하다는 데 동의하는 것입니다. 이 <strong>동의는 철회할 수 없습니다.</strong>`))}" />
			</div>
