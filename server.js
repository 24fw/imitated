...
async function init() {
	...
	print('위키엔진설치를 시작합니다');
	print('이메일 사용이 가능하므로 이메일 사용이 기본값입니다');
	print('사용전 readme파일을 꼭 읽으십시오.\n');
	
	// 호스팅 설정
	var hostconfig = {
		wikiname: [input('위키이름: ')],
		domain: [input("도메인: ")],
		host: input('호스트 주소(내부ip): '),
		port: input('포트 번호: '),
		skin: input('기본 스킨 이름: '),
		search_host: '127.5.5.5',
		search_port: '25005',
		send_email: [input('발송 이메일: ')],
		send_passwd: [input('발송 이메일 비밀번호: ')],
		owners: [input('소유자 닉네임: ')],
		search_autostart: 'true',
	};
